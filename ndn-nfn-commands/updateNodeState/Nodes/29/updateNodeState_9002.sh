...
do 
	...
	#2 --> 1	#2 --> 3	#2 --> 4	#2 --> 5
	B9001=$(echo $(ping -c 1 10.2.1.28 | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c 10.2.1.28 -p 8001 -u -t 1 | tail -1 | awk '{print $8}') |bc)
	B9003=$(echo $(ping -c 1 10.2.1.30 | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c 10.2.1.30 -p 8003 -u -t 1 | tail -1 | awk '{print $8}') |bc)
	B9004=$(echo $(ping -c 1 10.2.1.31 | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c 10.2.1.31 -p 8004 -u -t 1 | tail -1 | awk '{print $8}') |bc)
