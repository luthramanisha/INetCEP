...
do
	...
	processing=$(echo $var | awk '{print $3;}' )
	...
	#timestamp=$(echo $var | awk '{printll
	# $8;}' )
	...
	timeout 20 $CCNL_HOME/bin/ccn-lite-peek -s ndn2013 -u $ParentIP/$ParentPort -w 20 "" "call 9 /node/$ParentNodeName/nfn_service_$QueryType '$algorithm' '$processing' '$newRunID' 'QS' '$clientID' '$query' '$region' '$timestamp'/NFN" | $CCNL_HOME/bin/ccn-lite-pktdump -f 2
	if [ $? -eq 124 ]; then
	    # Timeout occurred
