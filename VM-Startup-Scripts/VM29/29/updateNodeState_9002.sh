...
do 
	...
	#2 --> 1	#2 --> 3	#2 --> 4	#2 --> 5
	B9001=$(echo $(ping -c 1 ${VMS[0]} | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c ${VMS[0]} -p 8001 -u -t 1 | tail -1 | awk '{print $8}') |bc)
	B9003=$(echo $(ping -c 1 ${VMS[2]} | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c ${VMS[2]} -p 8003 -u -t 1 | tail -1 | awk '{print $8}') |bc)
	B9004=$(echo $(ping -c 1 ${VMS[3]} | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c ${VMS[3]} -p 8004 -u -t 1 | tail -1 | awk '{print $8}') |bc)
	B9005=$(echo $(ping -c 1 ${VMS[4]} | tail -1| awk '{print $4 / 2}' | cut -d '/' -f 2) \* $(iperf -c ${VMS[4]} -p 8005 -u -t 1 | tail -1 | awk '{print $8}') |bc)
