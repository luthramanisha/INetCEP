...
/**
  * Created by Ali on 08/01/2017.
  */
object CCNHTTP extends App {
  ...
  val exampleContent = FileSystemDocument.documentWithPath("/home/veno/Thesis/omnetpp/omnetpp-4.5/doc/InstallGuide.pdf", node2.localPrefix)
  ...
  def makeHTTPCall() = {
    //Thread.sleep(3000)
    val startTime = System.currentTimeMillis
    println(s"Sending HTTP Request: https://tst.kgapi.bl.ch/solr/kim-portal.objects/query")
    val res = Http("https://tst.kgapi.bl.ch/solr/kim-portal.objects/query").param("q", "*:*").asString
    val runTime = System.currentTimeMillis - startTime
    println(s"RESULT:\n${res}\nTIME: (${runTime}ms)")
  }
}
